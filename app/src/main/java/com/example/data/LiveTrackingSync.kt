...
object LiveTrackingSync {
    ...
    /**
     * Continuous, always-on driver location (not tied to a specific trip) - keyed by the
     * driver's own stable ID, so it works the same on every device. Written by
     * LocationTrackingService while it runs in the background.
     */
    suspend fun pushContinuousDriverLocation(driverId: Int, lat: Double, lng: Double) {
        try {
            db.collection("driver_continuous_location")
                .document(driverId.toString())
                .set(mapOf("lat" to lat, "lng" to lng, "updatedAt" to System.currentTimeMillis()))
                .await()
        } catch (e: Exception) {
            // Best-effort only.
        }
