...
interface CommissionDao {
    ...
    @Query("SELECT * FROM commissions WHERE cloudId = :cloudId LIMIT 1")
    suspend fun getCommissionByCloudIdSync(cloudId: String): CommissionPayment?
}
...
interface JobDao {
    ...
    @Query("SELECT * FROM job_profiles WHERE cloudId = :cloudId LIMIT 1")
    suspend fun getJobByCloudIdSync(cloudId: String): JobProfile?
}

@Database(
    entities = [User::class, Load::class, ChatMessage::class, CommissionPayment::class, JobProfile::class],
    version = 7, // Bump version: added cloudId to CommissionPayment/JobProfile for cross-device sync
    exportSchema = false
)
