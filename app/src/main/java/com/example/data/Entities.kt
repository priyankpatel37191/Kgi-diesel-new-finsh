...
data class User(
    @PrimaryKey val id: Int = 0, // now derived from phone number at creation time - see stableIdFromPhone()
...
data class Load(
    ...
    val cloudId: String = "", // stable cross-device key (UUID) - the local `id` above can differ per phone
    val shipperId: Int,
...
data class CommissionPayment(
    @PrimaryKey(autoGenerate = true) val id: Int = 0,
    val cloudId: String = "", // stable cross-device key, same pattern as Load.cloudId
    val driverId: Int,
    val shipperId: Int,
    val loadId: Int,
    val amount: Double,
    ...
    // UNPAID -> SUBMITTED (user entered UTR, awaiting admin check) -> VERIFIED (unlocks) or REJECTED (must resubmit)
    val verificationStatus: String = "UNPAID",
