    ...
    suspend fun pushLoad(load: Load) {
        ...
        try {
            val data = mapOf(
                ...
                "pickupLat" to load.pickupLat,
                "pickupLng" to load.pickupLng,
                "dropLat" to load.dropLat,
                "dropLng" to load.dropLng,
    ...
    private fun mapToLoad(cloudId: String, d: Map<String, Any?>): Load? {
        return try {
            Load(
                ...
                pickupLat = (d["pickupLat"] as? Number)?.toDouble() ?: 0.0,
                pickupLng = (d["pickupLng"] as? Number)?.toDouble() ?: 0.0,
                dropLat = (d["dropLat"] as? Number)?.toDouble() ?: 0.0,
                dropLng = (d["dropLng"] as? Number)?.toDouble() ?: 0.0,
