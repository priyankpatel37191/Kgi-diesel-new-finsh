...
class MainViewModel(
    ...
    suspend fun searchLiveOsmOutposts(
        ...
        if (results.isEmpty()) {
            val fallback = getNearbyServices(categoryId, city, pincode, state, area, centerLat, centerLng)
            Log.d("OverpassAPI", "Overpass returned 0 results. Using fallback catalog count: ${fallback.size}")
